...
QUnit.log(function (data) {
    ...
    testState.currentAssertions.push(data);
    if (!data.result) {
...
QUnit.testDone(function (data) {
    ...
    data.assertions = testState.currentAssertions;
...
jqUnit.writeJunitXml = function (filename) {
    var testsuite = xmlbuilder.create("testsuite");
    testsuite.attribute("tests", testState.tests.length);
    fluid.each(testState.tests, function (testData) {
        var testcase = testsuite.element("testcase", { "classname": testData.module, "name": testData.name });
        if (testData.failed > 0) {
            var message = "";
            fluid.each(testData.assertions, function (assertion) {
                if (!assertion.result) {
                    message += "Message: " + assertion.message + "\n";
                    if (assertion.expected !== undefined) {
                        message += "Expected: " + util.format(assertion.expected) + "\n";
                        message += "Actual: " + util.format(assertion.actual) + "\n";
                    }
                    if (assertion.source) {
                        message += assertion.source + "\n";
                    }
                }
            });
            testcase.element("failure").text(message);
        }
    });
    fs.writeFileSync(filename, testsuite.end({ pretty: true, indent: "  ", newline: "\n" }));
}
...
jqUnit.allTestsDone = function (data) {
    ...
    jqUnit.writeJunitXml("test-result.xml");
